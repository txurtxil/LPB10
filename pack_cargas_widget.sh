...
# ========== 1. Umbral por tiempo en main.dart (3 sitios) ==========
python3 - <<'PYEOF'
import io, sys
d = "/tmp/pruebatest/main.dart"
s = io.open(d, encoding='utf-8').read()

# Helper: minutos entre startTs y endTs
# 1a. Filtro en load() (linea 1377)
...
if s.count(old1) != 1:
    sys.exit("ABORT: ancla filtro load x%d" % s.count(old1))
s = s.replace(old1, new1, 1)

# 1b. startSession (linea ~1395): al cerrar la vieja antes de abrir nueva
...
new2 = '''      final open = sessions.last;
      ...
      if (gain >= 0.3 && mins >= 3.0) {
        ...
        sessions.removeLast(); // parpadeo: poco tiempo o sin ganancia real
      }'''
if s.count(old2) != 1:
    sys.exit("ABORT: ancla startSession x%d" % s.count(old2))
s = s.replace(old2, new2, 1)

# 1c. reconcileOpenSession (linea ~1425)
...
if s.count(old3) != 1:
    sys.exit("ABORT: ancla reconcile x%d" % s.count(old3))
s = s.replace(old3, new3, 1)

io.open(d, 'w', encoding='utf-8').write(s)
print("[ok] main.dart: umbral por tiempo (>=3 min y >=0.3%) en 3 sitios")
...
echo "[ok] etiqueta Ajustes actualizada (ES/EN)"

# ========== 3. Drawable de fondo redondo oscuro para el boton ==========
...
XEOF
echo "[ok] drawable widget_refresh_bg.xml creado"

# ========== 4. Boton del widget mas grande y oscuro ==========
python3 - <<'PYEOF'
import io, sys
...
new = '''        <ImageButton
            ...
            android:contentDescription="Actualizar" />'''
if s.count(old) != 1:
    sys.exit("ABORT: ancla ImageButton widget x%d" % s.count(old))
s = s.replace(old, new, 1)
io.open(p, 'w', encoding='utf-8').write(s)
print("[ok] widget: boton 40dp, fondo redondo oscuro, icono blanco")
PYEOF

echo "[i] Verificacion:"
echo -n "  umbral por tiempo (mins >= 3.0, debe ser 3): "; grep -c "mins >= 3.0" lib/main.dart
echo -n "  etiqueta ES: "; grep -c "Ajustes, logs y backups" lib/l10n/app_es.arb
echo -n "  drawable existe: "; ls android/app/src/main/res/drawable/widget_refresh_bg.xml >/dev/null 2>&1 && echo OK
echo -n "  boton 40dp: "; grep -c 'layout_width="40dp"' android/app/src/main/res/layout/battery_widget.xml
